...
# -*- coding: utf-8 -*-
"""
Sentiment Analyst 的 MemoryManager。

说明：
    - 继承自 BaseAnalystMemoryManager
    - 使用 sentiment_summary.j2 Prompt 模板生成 7 日窗口的结构化 summary
"""
...
class SentimentMemoryManager(BaseAnalystMemoryManager):
    """Sentiment Analyst 的 MemoryManager。"""

    def __init__(self, db_helper: Any, date_resolver: Optional[Any] = None) -> None:
        super().__init__(analyst_type="sentiment", db_helper=db_helper, date_resolver=date_resolver)
    ...
    def _generate_summary_with_llm(
        ...
        """使用 LLM 生成 7 日窗口的结构化 Sentiment summary。
        ...
        """
        # 1. 加载 Prompt 模板
        prompt_path = Path(__file__).parent / "prompts" / "sentiment_summary.j2"
        ...
        try:
            ...
            # 验证关键字段是否为空
            if not parsed_json.get("symbol") or not parsed_json.get("executive_synthesis"):
