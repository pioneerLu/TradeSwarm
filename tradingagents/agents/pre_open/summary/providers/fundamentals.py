...
def load_fundamentals_summary(conn, state: AgentState) -> AnalystMemorySummary:
    symbol = state["company_of_interest"]
    trade_date = state["trade_date"]
    trading_session = state.get("trading_session", "post_close")
    return {
        "today_report": query_today_report(
            conn, "fundamentals", symbol, trade_date, "Fundamentals Analysis Report"
        ),
        "history_report": query_history_report(
            conn, "fundamentals", symbol, trade_date, trading_session, "Fundamentals History Summary"
        ),
    }
