...
从 Pre-Open 的 AgentState 解析出 QuantConnect 可执行的信号，不执行实际交易。
输出结构化信号供 run_signal_export 和 QuantConnect 使用。
"""
...
def resolve_signal(
    ...
    Args:
        ...
        is_holding: 当前是否持有该标的
        data_adapter: 数据适配器（用于交易日计算）

    Returns:
        信号 dict，包含 action, target_pct, execution_date, entry_type, entry_price, reason 等
    """
    ...
    # 5) 仓位融合：risk 只做上限约束，不放大 trader 仓位
    ...
    return {
        ...
        "reason": trader_summary or "由 Trader 直接生成可执行计划，并由 Risk 约束",
        "is_exploratory": False,
        "execution_date": next_trading_day,
    }
